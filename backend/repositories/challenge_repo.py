...
class ChallengeRepository:
    ...
    async def get_all(self):
        result = await self.session.execute(select(DailyChallenge).order_by(DailyChallenge.challenge_date.desc()))
        return result.scalars().all()

    async def get_by_id(self,challenge_id: int):
        result = await self.session.execute(select(DailyChallenge).where(DailyChallenge.id == challenge_id))
        return result.scalar_one_or_none()

    async def get_by_date(self,challenge_date: date):
        result = await self.session.execute(select(DailyChallenge).where(DailyChallenge.challenge_date == challenge_date))
        return result.scalar_one_or_none()
