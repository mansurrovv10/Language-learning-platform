...
from backend.schemas.achievement_schema import (AchievementCreate,AchievementUpdate,AchievementResponse,UserAchievementResponse)
...
def get_achievement_service(session: AsyncSession = Depends(get_session)):
...
async def get_achievements(service: AchievementService = Depends(get_achievement_service),
...
async def award_achievement(achievement_id: int,user_id: uuid.UUID,
    service: AchievementService = Depends(get_achievement_service),
...
async def update_achievement(achievement_id: int,data: AchievementUpdate,
    service: AchievementService = Depends(get_achievement_service),
    current_user: UserProfile = Depends(require_admin)):
