...
async def get_progress(service: ProgressService = Depends(get_progress_service),
    current_user: UserProfile = Depends(require_admin)):
    return await service.get_all()


@router.get("/user/{user_id}",response_model=list[ProgressResponse])
async def get_user_progress(user_id: uuid.UUID,service: ProgressService = Depends(get_progress_service),
...
async def get_lesson_progress(lesson_id: int,service: ProgressService = Depends(get_progress_service),
    current_user: UserProfile = Depends(require_admin)):
    return await service.get_by_lesson(lesson_id)


@router.get("/{progress_id}",response_model=ProgressResponse)
async def get_progress_by_id(progress_id: int,service: ProgressService = Depends(get_progress_service),
    current_user: UserProfile = Depends(get_current_user)):
...
async def update_progress(progress_id: int,data: ProgressUpdate,
    ...
    return await service.update(progress_id,data.completed,data.score)


@router.delete("/{progress_id}")
async def delete_progress(progress_id: int,service: ProgressService = Depends(get_progress_service),
    current_user: UserProfile = Depends(require_admin)):
